...
exports.runTextExtractTest = function()
{
    ...
    function* main()
    {
        ...
        var output_path = "../../TestFiles/Output/";
        ...
        try  
        {
            ...
            var cnt = yield txt.getNumLines();

            if(example1_basic){
                var wordCount = yield txt.getWordCount();
                console.log("Word Count: " + wordCount);
                var text = yield txt.getAsText();
            ...
            if(example2_xml){
                var text = yield txt.getAsXML(PDFNet.TextExtractor.XMLOutputFlags.e_words_as_elements | PDFNet.TextExtractor.XMLOutputFlags.e_output_bbox | PDFNet.TextExtractor.XMLOutputFlags.e_output_style_info);
                console.log("- GetAsXML  --------------------------" + text);
                console.log("-----------------------------------------------------------");
            }

            if(example3_wordlist){
                var line = yield txt.getFirstLine();
                var word;
            ...
            if(example4_advanced){
                ...
                for(var line = yield txt.getFirstLine(); yield line.isValid(); line = yield line.getNextLine()){
                    ...
                    for(var word = yield line.getFirstWord(); yield word.isValid(); word = yield word.getNextWord()){
