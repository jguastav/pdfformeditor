(function () {
	...
	var runCustomScript = function(pdfDoc, layersContextID, windowCoord, pageIndex, documentViewer, Annotations, annotManager)
	{
		...
		function* ProcessPaths(opr, pointList, currTransMtx, pageIndex) {
			...
			for (var opr_index = 0; opr_index < opr.length; ++opr_index)
			{
                ...
                case PDFNet.Element.PathSegmentType.e_moveto:
                    ...
                    var x1 = pointList[point_index]; ++point_index;
                    var y1 = pointList[point_index]; ++point_index;
                    var pagePoint = yield currTransMtx.mult(x1, y1);
				...
				case PDFNet.Element.PathSegmentType.e_cubicto:
					...
					var pagePoint = yield currTransMtx.mult(x3, y3);
					yield * DrawPointAnnot(pageIndex, pagePoint.x, pagePoint.y);
					break;
		...
		function* ExtractElements(page_reader) {
			var elementArray = [];
			for (var element = (yield page_reader.next()); element !== null; element = (yield page_reader.next())) // Read page contents
			{
					...
					case PDFNet.Element.Type.e_path:  // Process path data
						{
							...
							var elementBBox = yield element.getBBox();
							var retObj = {name:"path", type:elemType, ctm:ctm, operators:opr, points:points, bbox:elementBBox};
					...
					case PDFNet.Element.Type.e_image: // Process image data
						{
							...
							var retObj = {name:"image", type:elemType, num: elementNum, ctm:ctm, bbox:elementBBox};
							elementArray.push(retObj);
						}
						break;
		...
		function* main()
		{
			...
			try 
			{
				...
				if(pageElementData == undefined){
					...
					var layersContext = new PDFNet.OCGContext(layersContextID);
