...
exports.runInteractiveFormsTest = function()
{
    ...
    function * CreateCheckmarkAppearance(doc, style)
    {
        ...
        switch (style) 
        {
            case PDFNet.CheckStyle.e_circle: symbol = '\154'; break;
            case PDFNet.CheckStyle.e_diamond: symbol = '\165'; break;
            case PDFNet.CheckStyle.e_cross: symbol = '\65'; break;
            case PDFNet.CheckStyle.e_square: symbol = '\156'; break;
            case PDFNet.CheckStyle.e_star: symbol = '\110'; break;
                // ...
            ...
            else // Manual flattening
            {
                ...
                for (var pitr = yield doc4.getPageIterator(); (yield pitr.hasNext()); (yield pitr.next()))  
                {
                    ...
                    {   // Look for all widget annotations (in reverse order)
                        for (var i = parseInt(yield annots.size())-1; i>=0; --i)
                        {
